...
根据 connectedhomeip 仓库中的实现，一些cluster需要由应用程序定义的delegate来处理特定的数据和操作。
为了能在应用程序中灵活的实现此功能，esp-matter在创建cluster的API中提供了delegate的初始化回调函数。
应用程序需要以delegate-impl类的形式定义其数据和操作，并在创建cluster或device type的时候设置delegate。
