...
void zigbee_bridge_contact_state_change_handler(uint16_t zb_addr, uint8_t zb_endpoint, bool state)
{
    ...
    send_onoff_command_to_binding_table(endpoint_id, !state);
}

void zigbee_bridge_on_off_report_handler(uint16_t zb_addr, uint8_t zb_endpoint, bool on_off)
{
    ESP_LOGI(TAG, "onoff Attribute Report command is received, src addr 0x%x, src ep %d", zb_addr, zb_endpoint);
    uint16_t endpoint_id = 2;
    ...
    send_onoff_command_to_binding_table(2, on_off);
}
