...
void Driver::_applyTunnableWhite()
{
    ...
    // Apply the PWMs
    ESP_LOGD(TAG, "Updating white: Warm 0x%x, Cold 0x%x", ticksWw / 256, ticksCw / 256);
...
/**
 * @brief Update lights according to the color mode
 */
void Driver::updateLight()
{
    printf("updateLight----------\n");
...
void Driver::indicate_update()
{
    printf("indicate_update----------\n");
    if (_mode == LIGHT_DRIVER_MODE_RGB_HSV) {
        if (_on) {
            printf("LIGHT_DRIVER_MODE_RGB_HSV----------\n");
            _applyRgb();
...
/**
 * @brief Turn on the device for indicate
 */
void Driver::indicateOn()
{
    _on = true;
    printf("indicateOn----------\n");
    indicate_update();
