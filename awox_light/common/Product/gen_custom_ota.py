...
def add_app_header(args):
    ...
    print('packed_file file is: {}'.format(packed_file))
    ...
    print('signed_file file is: {}'.format(signed_file))

    #step3: add the idf firmware image header
    if (header_version[args.header_ver] == 3):
        print("test1")
        shutil.copyfile(signed_file, ''.join([signed_file,'.add_header']))
        print("test2")
        app_header_file = ''.join([signed_file,'.add_header'])
        print('app_header_file file is: {}'.format(app_header_file))
        with open(app_header_file, 'rb+') as src_f:
            ...
            print('add_header_file is: {}'.format(app_header_file))

def compress_bin(args):
    compress_type = args.compress_type
    ...
    compressed_app_directory = 'custom_ota_binaries'
    if os.path.exists(compressed_app_directory):
        shutil.rmtree(compressed_app_directory)

    os.mkdir(compressed_app_directory)
    print('The compressed file will store in {}'.format(compressed_app_directory))

    #step1: compress
    if compress_type == 'xz':
        xz_compress(compressed_app_directory, os.path.abspath(src_file))
    ...
    # shutil.copyfile(signed_file, ''.join([signed_file,'.no-header']))
...
def main():
    ...
    # subparser for compress_bin
    compress_bin = subparsers.add_parser(
        "compress_bin", help="Download an image to RAM and execute"
    )
    compress_bin.add_argument('--sign_key', nargs = '?',
            default='', help='the sign key used for secure boot')
    compress_bin.add_argument('-fv', '--fw_ver', nargs='?',
            default='', help='the version of the compressed data(this field is deprecated in v3)')

    # subparser for add_app_header
    add_app_header = subparsers.add_parser(
        "add_app_header", help="add app header to use native esp_ota_* & esp_https_ota_* APIs"
    )
    add_app_header.add_argument('--signed', action="store_true",
            help='the file already signed')
    ...
    print("gen_custom_ota.py v%s" % SCRIPT_VERSION)
