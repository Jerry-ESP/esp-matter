...
static void app_event_cb(const ChipDeviceEvent *event, intptr_t arg)
{
    ...
    case chip::DeviceLayer::DeviceEventType::kCommissioningComplete:
        ...
        commission_indicator(COMMISSIONING_SUCCESS);
        break;

    case chip::DeviceLayer::DeviceEventType::kFailSafeTimerExpired:
        ESP_LOGI(TAG, "Commissioning failed, fail safe timer expired");
        commission_indicator(COMMISSIONING_FAILED);
        break;

    case chip::DeviceLayer::DeviceEventType::kCommissioningSessionStarted:
        ESP_LOGI(TAG, "Commissioning session started");
        commission_indicator(COMMISSIONING_START);
        break;
                                       ...
                                       uint8_t effect_variant, void *priv_data)
{
    ...
    switch (type) {
        case identification::START:
            err = identify_indicator(IDENTIFY_START);
            break;

        case identification::STOP:
            err = identify_indicator(IDENTIFY_STOP);
            break;

        case identification::EFFECT:
            switch (effect_id) {
                case (int)Identify::EffectIdentifierEnum::kBlink:
                    err = identify_indicator(IDENTIFY_EFFECT_BLINK);
                    break;

                case (int)Identify::EffectIdentifierEnum::kBreathe:
                    err = identify_indicator(IDENTIFY_EFFECT_BREATH);
                    break;

                case (int)Identify::EffectIdentifierEnum::kOkay:
                    err = identify_indicator(IDENTIFY_EFFECT_OK);
                    break;

                case (int)Identify::EffectIdentifierEnum::kChannelChange:
                    err = identify_indicator(IDENTIFY_EFFECT_CHANGE);
                    break;

                case (int)Identify::EffectIdentifierEnum::kFinishEffect:
                    err = identify_indicator(IDENTIFY_EFFECT_FINISH);
                    break;

                case (int)Identify::EffectIdentifierEnum::kStopEffect:
                    err = identify_indicator(IDENTIFY_EFFECT_STOP);
                    break;
...
extern "C" void app_main()
{
    ...
    ABORT_APP_ON_FAILURE(err == ESP_OK, ESP_LOGE(TAG, "Failed to init driver, err:%d", err));
