...
static Driver s_driver = Driver(PWM_CW_PIN,
                                PWM_WW_PIN,
                                PWM_RED_PIN,
                                PWM_GREEN_PIN,
                                PWM_BLUE_PIN);
...
static esp_err_t light_driver_set_hue_and_saturation(uint16_t hue, uint16_t saturation)
{
    esp_err_t err = ESP_OK;
    ESP_LOGI(TAG, "Set hue: %d and saturation %d", hue, saturation);

    s_driver.setHueAndSaturation(hue, saturation);
    return err;
}
...
static esp_err_t light_matter_set_hue(uint8_t hue)
{
    ...
    uint16_t value = REMAP_TO_RANGE(light_current_state.saturation, MATTER_SATURATION, STANDARD_SATURATION);

    return light_driver_set_hue_and_saturation(hue, value);
}
...
static esp_err_t light_matter_set_saturation(uint8_t saturation)
{
    ...
    return light_driver_set_hue_and_saturation(light_current_state.hue, value);
}
