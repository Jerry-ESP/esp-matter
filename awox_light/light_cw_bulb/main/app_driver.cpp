...
static RgbTw::Driver s_driver = RgbTw::Driver(PWM_CW_PIN,
                                            PWM_WW_PIN,
                                            PWM_RED_PIN,
                                            PWM_GREEN_PIN,
                                            PWM_BLUE_PIN);
...
static esp_err_t light_driver_set_x(uint16_t x, uint8_t level)
{
    esp_err_t err = ESP_OK;
    ESP_LOGI(TAG, "Set color X %d, level %d", x, level);
...
static esp_err_t light_driver_set_y(uint16_t y, uint8_t level)
{
    esp_err_t err = ESP_OK;
    ESP_LOGI(TAG, "Set color Y %d, level %d", y, level);
