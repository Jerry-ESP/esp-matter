...
typedef enum {
    ...
    INDICATOR_MODE_SOLID, /* single color */
...
static esp_err_t indicator_trigger_with_count(indicator_config_t *config, uint8_t count)
{
    if (!config) {
    ...
    return lightbulb_basic_effect_start(&effect_config);
}
...
static esp_err_t indicator_trigger_lightbulb(indicator_config_t *config)
{
    ...
    if (config->mode == INDICATOR_MODE_BREATHE || config->mode == INDICATOR_MODE_BLINK) {
        ...
        lightbulb_rgb2hsv(config->color.r, config->color.g, config->color.b, &effect_config.hue, &effect_config.saturation, &calculate_value);
        return lightbulb_basic_effect_start(&effect_config);
    } else if (config->mode == INDICATOR_MODE_SOLID) {
        esp_err_t ret;
        // In solid mode，using lightbulb_basic_effect to recording this indicator and restore status after timeout.
        // so always use max_brightness in solid mode.
        lightbulb_effect_config_t effect_config = {
            .effect_type = (lightbulb_effect_t)EFFECT_BLINK,
            .mode = (lightbulb_works_mode_t)config->color.color_select,
            .cct = config->color.cct,
            .min_value_brightness = config->max_brightness, // always use max_brightness in solid mode
            .max_value_brightness = config->max_brightness,
            .effect_cycle_ms = config->speed,
            .total_ms = config->total_ms,
            .user_cb = led_indicator_lightbulb_effect_callback,
        ...
        if (!config->total_ms) { // always show this color without timeout
            uint16_t hue = 0;
            uint8_t saturation, value = 0;
            lightbulb_rgb2hsv(config->color.r, config->color.g, config->color.b, &hue, &saturation, &value);
            /*
            * lightbulb_basic_effect_start() will not update the driver value. If light effect is always solid, driver_update_callback should update the driver and attribute.
            * Update value after lightbulb_basic_effect_stop()
            */
            lightbulb_basic_effect_stop();
        } else {
            ret = lightbulb_basic_effect_start(&effect_config);
        }
        ...
        return lightbulb_set_switch(light_status);
    }
